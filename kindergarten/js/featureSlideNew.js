function initFeatureSlideNew(columnId) {
    var domRoot = document.getElementById('fSlideBlock' + columnId);
    if (!domRoot) return;
    domRoot.list = [];
    ...
    for (var i in children) {
        ...
        if (domItem && domItem.className == 'modFeatureSlidePreview') {
            domRoot.list.push(domItem);
            domItem.offset = offset;
            offset++;
        }
    }
    var domList = document.getElementById('fSlideItems' + columnId);
    if (!domList) return;
    domList.innerHTML = '';
    domList.items = [];
    ...
    var domPreviousBtn = document.getElementById('fSlidePrevious' + columnId);
    var domNextBtn = document.getElementById('fSlideNext' + columnId);
