<template>
	<div class="apply-albumvideo">
		<div class="apply-album apply-av">
			...
			<UploadImg roleType="album" applyType="actor"/>
		...
		<div slot="body">
			...
			<!-- <span @click="clearTimeHandler">
				<router-link class="go-realname" tag="span" to="/ActorRealname">前往实名认证</router-link>
			</span> -->
		</div>
		<div slot="footer">
			<router-link class="modal-jump" tag="button" to="/ActorEdit">跳过({{times}})</router-link>
		</div>
		</ModalItem>
		<ModalItem
			v-if="imgLengthFlag"
		...
		methods:{
			...
			timesChange(){
				this.timer = setInterval(()=>{
					this.times--;
					if(this.times<=0){
						window.location.href="#/ActorEdit"
			...
			submitApplyHandler(){
				if(this.showImgUrl.length<3 || this.showvideoUrl==''){
